...
var selectedLabels= new Array(); //keeps track of the selected elements
...
function select(loc){
	for(t=0;t<tiles.length;t++){
		tiles[t].selected=0;
		if (tiles[t].Contains(loc)){
			if( selectedLabels.length == 0){
				selectedLabels[0]=tiles[t];
				selectedTile=tiles[t];
				selectedTile.selected=1;
			
			}else if (selectedLabels.length == 1){
				if(selectedLabels[0].label == tiles[t].label && selectedLabels[0] != tiles[t]){
					//time to get em outa there
					tiles.splice(t,1);
					tiles.splice(tiles.findIndex(selectedLabels[0]),1);
...
function draw(){    
	...
	context.globalAlpha = tiles.length ? 0.2: 1.0;
...
function init(){
	...
	while(counter<numTiles){
		tiles[counter].faceUp=20;
          	tiles[counter+numTiles] = new Tile(counter+numTiles,images[counter],images[counter].label); 
          	counter=counter+1;
    	}
